...
def main(mode, data_dir, image_path, image_dir):
    ...
    pred_label, logits, regularization_loss = inference(image, keep_probability)
    ...
    if mode == 'train':
        ...
        # Compute loss
        loss = tf.add(tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits,labels=annotation,name="entropy")), REGULARIZATION_SCLAE*regularization_loss)
